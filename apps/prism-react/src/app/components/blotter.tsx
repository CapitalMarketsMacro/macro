...
export function Blotter() {
  ...
  const effectiveColumns = useMemo(
    () => (rollupOn && rollup ? applyRollupToColumns(columns, rollup) : columns),
    [columns, rollup, rollupOn],
  );

  const gridOptions = useMemo<GridOptions>(() => {
    const r = rollupOn ? rollup : null;
    const rollupOpts = r ? rollupGridOptions(r) : {};
    ...
    return {
      ...
      processAutoGeneratedColumnDefs: (params) => {
        ...
        return r ? applyRollupToColumns(defs, r) : defs;
      },
    };
  }, [columnMode, formats, rollup, rollupOn]);
  ...
  useEffect(() => {
    ...
    const feed = new BlotterFeed(source, ops, (rec) => {
      ...
      // An explicit source roll-up wins; otherwise suggest one from the inferred columns so the
      // toolbar toggle works on any payload with plausible grouping fields.
      if (!source.rollup) {
