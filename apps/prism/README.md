...
For a large SOW, set **Top N** in the ad-hoc dialog to send AMPS's `top_n` option and bound the initial rows returned by `sow_and_subscribe`. Without `skip_n`, `top_n` applies only to that initial SOW query; matching live updates continue after the snapshot. **Order by** sends the AMPS `OrderBy` header and accepts a comma-separated expression such as `/orderDate DESC, /customerName ASC`; each field may specify `ASC` (the default), `DESC`, and the optional `TEXT` sort hint. See the AMPS documentation for [managing result sets](https://crankuptheamps.com/docs/amps-user-guide/sow-queries/managing-result-sets) and the [`OrderBy` / `top_n` protocol fields](https://crankuptheamps.com/docs/amps-command-reference/protocol/amps-protocol#amps-message-header---sorted-by-name).
