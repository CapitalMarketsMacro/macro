import { Component, OnDestroy, computed, effect, inject, signal, viewChild } from '@angular/core';
import { ActivatedRoute, Router, RouterLink } from '@angular/router';
import { toSignal } from '@angular/core/rxjs-interop';
import { map } from 'rxjs/operators';
import type { ColDef, GetRowIdParams, GridOptions } from 'ag-grid-community';
import { Button } from 'primeng/button';
import { Tag } from 'primeng/tag';
import { SelectButton } from 'primeng/selectbutton';
import { Message } from 'primeng/message';
import { Popover } from 'primeng/popover';
import { InputText } from 'primeng/inputtext';
import { FormsModule } from '@angular/forms';
import { MacroAngularGrid } from '@macro/macro-angular-grid';
import type { ColumnFormatMap } from '@macro/macro-grid-format';
import {
  BlotterFeed,
  MODE_LABELS,
  TRANSPORT_LABELS,
  applyInferredFormats,
  applyRollupToColumns,
  completeRollup,
  inferColumns,
  rollupGridOptions,
  suggestRollup,
  type BlotterMode,
  type BlotterSource,
  type ColumnMode,
  type FeedState,
  type FeedStatus,
  type GridOps,
  type RollupConfig,
} from '@macro/prism-core';
import { DataSourceRepository } from '../../services/data-source-repository.service';
import { ActiveSourceService } from '../../services/active-source.service';
import { AdHocSourceDialogComponent } from '../adhoc-source-dialog/adhoc-source-dialog.component';

type Row = Record<string, unknown>;

@Component({
  selector: 'app-blotter',
  standalone: true,
  imports: [RouterLink, FormsModule, MacroAngularGrid, Button, Tag, SelectButton, Message, Popover, InputText, AdHocSourceDialogComponent],
  templateUrl: './blotter.component.html',
  styleUrl: './blotter.component.css',
})
export class BlotterComponent implements OnDestroy {
  private readonly grid = viewChild(MacroAngularGrid);
  private readonly picker = viewChild.required<Popover>('picker');
  private readonly dialog = viewChild.required(AdHocSourceDialogComponent);
  private readonly repo = inject(DataSourceRepository);
  private readonly activeSource = inject(ActiveSourceService);
  private readonly route = inject(ActivatedRoute);
  private readonly router = inject(Router);

  readonly transportLabels = TRANSPORT_LABELS;
  readonly modeLabels = MODE_LABELS;

  readonly source = signal<BlotterSource | null>(null);
  readonly columns = signal<ColDef[]>([]);
  readonly formats = signal<ColumnFormatMap>({});
  readonly columnMode = signal<ColumnMode>('infer');
  readonly showGrid = signal(true);

  /** Active roll-up config: the source's own, else a suggestion inferred from the first record. */
  readonly rollup = signal<RollupConfig | null>(null);
  /** Whether the roll-up view is on (seeded from `source.rollup.enabled`, toggled from the toolbar). */
  readonly rollupOn = signal(false);
  /** A usable roll-up exists (a source config with a blank hierarchy waits for the suggestion). */
  readonly hasRollup = computed(() => (this.rollup()?.groupBy.length ?? 0) > 0);
  /** The config to actually group by, or null when the view is off / the config has no hierarchy. */
  private readonly activeRollup = computed<RollupConfig | null>(() =>
    this.rollupOn() && this.hasRollup() ? this.rollup() : null,
  );

  /** Columns actually bound to the grid — decorated with rowGroup/aggFunc when rolled up. */
  readonly effectiveColumns = computed<ColDef[]>(() => {
    const cols = this.columns();
    const r = this.activeRollup();
    return r ? applyRollupToColumns(cols, r) : cols;
  });

  /** Filter text for the source picker popover. */
  readonly pickerFilter = signal('');

  /** Sources grouped by category, narrowed by the picker's filter text. */
  readonly pickerGroups = computed(() => {
    const q = this.pickerFilter().trim().toLowerCase();
    const groups = this.repo.groupedByCategory();
    if (!q) return groups;
    return groups
      .map((g) => ({
        category: g.category,
        sources: g.sources.filter(
          (s) =>
            s.name.toLowerCase().includes(q) ||
            s.category.toLowerCase().includes(q) ||
            s.topic.toLowerCase().includes(q) ||
            this.transportLabels[s.transport].toLowerCase().includes(q),
        ),
      }))
      .filter((g) => g.sources.length > 0);
  });

  /** Mirror of the framework-free feed state into a signal so the template stays reactive. */
  private readonly feedState = signal<FeedState>({ status: 'idle', rowCount: 0, msgsPerSec: 0, error: null });

  readonly columnModeOptions = [
    { label: 'Infer', value: 'infer' as ColumnMode },
    { label: 'Auto (v36)', value: 'auto-gen' as ColumnMode },
  ];

  /** Seed the wrapper's format store only on the infer path; auto-gen formats via the callback. */
  readonly seedFormats = computed<ColumnFormatMap>(() => (this.columnMode() === 'infer' ? this.formats() : {}));

  readonly gridOptions = computed<GridOptions>(() => {
    const r = this.activeRollup();
    const rollupOpts: GridOptions = r
      ? {
          ...rollupGridOptions(r),
          // The wrapper's status-bar Grouping toggle / the row-group panel can clear every group
          // inside the grid — flip the roll-up view off with them so the toolbar stays honest
          // (and the remount removes the grand-total footer + unhides the group columns).
          onColumnRowGroupChanged: (e) => {
            // Only user gestures (status-bar toggle → 'api', panel drag → 'uiColumnDragged', …).
            // Our own columnDefs pushes arrive as 'gridOptionsChanged' and must not flip the view
            // (e.g. the transient empty-columns push while a feed restarts).
            if (e.source === 'gridOptionsChanged' || e.source === 'gridInitializing') return;
            if (!this.rollupOn() || e.api.getRowGroupColumns().length > 0) return;
            setTimeout(() => {
              this.rollupOn.set(false);
              this.restartGrid();
            }, 0);
          },
        }
      : {};
    if (this.columnMode() !== 'auto-gen') return rollupOpts;
    const formatDefs = applyInferredFormats(this.formats());
    return {
      ...rollupOpts,
      autoGenerateColumnDefs: { objectValues: 'flatten', nullishValues: 'include' },
      // Auto-gen columns never pass through `columns` — decorate the generated defs instead.
      // Read the signal at call time: generation runs synchronously with the first data batch,
      // before change detection re-delivers the config completed in onFirstRecord.
      processAutoGeneratedColumnDefs: (params) => {
        const defs = formatDefs(params) ?? params.columnDefs;
        const current = this.activeRollup();
        return current ? applyRollupToColumns(defs, current) : defs;
      },
    };
  });

  private feed?: BlotterFeed;
  private feedUnsub?: () => void;

  /** `?source=<id>` from the URL — the deep-link / layout-persisted source for this instance. */
  private readonly routeSourceId = toSignal(
    this.route.queryParamMap.pipe(map((pm) => pm.get('source'))),
    { initialValue: this.route.snapshot.queryParamMap.get('source') },
  );

  /** Reactive resolution of the URL id against the (async-loaded) repo + the in-memory active source. */
  private readonly resolvedSource = computed<BlotterSource | null>(() => {
    const id = this.routeSourceId();
    const all = this.repo.all(); // re-resolve once the catalog finishes loading
    if (id) return all.find((s) => s.id === id) ?? this.activeSource.active();
    return this.activeSource.active();
  });

  /** Last source id we (re)built a feed for; guards the effect against redundant restarts. */
  private appliedId: string | null | undefined = undefined;

  constructor() {
    // Switch the blotter in place whenever the resolved source changes (URL nav from the picker,
    // deep link, or catalog load). Each switch tears down the old feed and remounts the grid.
    effect(() => {
      const src = this.resolvedSource();
      const id = src?.id ?? null;
      if (id === this.appliedId) return;
      this.appliedId = id;
      this.applySource(src);
    });
  }

  readonly getRowId = (params: GetRowIdParams): string => {
    const src = this.source();
    const data = params.data as Row;
    if (src?.mode === 'append') return String(data['__id']);
    return String(data[src?.keyField ?? '__id']);
  };

  get status(): FeedStatus {
    return this.feedState().status;
  }
  get rowCount(): number {
    return this.feedState().rowCount;
  }
  get msgsPerSec(): number {
    return this.feedState().msgsPerSec;
  }
  get error(): string | null {
    return this.feedState().error;
  }

  statusSeverity(): 'success' | 'danger' | 'warn' | 'secondary' {
    switch (this.status) {
      case 'live':
        return 'success';
      case 'error':
        return 'danger';
      case 'connecting':
      case 'snapshot-loading':
        return 'warn';
      default:
        return 'secondary';
    }
  }

  ngOnDestroy(): void {
    this.feedUnsub?.();
    this.feed?.stop();
  }

  setColumnMode(mode: ColumnMode): void {
    if (mode === this.columnMode()) return;
    this.columnMode.set(mode);
    this.restartGrid(); // rebuild the grid with the new column strategy
  }

  /** Flat tape ⇄ rolled-up view. Remounts the grid so grouping options settle at creation. */
  toggleRollup(): void {
    if (!this.hasRollup()) return;
    this.rollupOn.set(!this.rollupOn());
    this.restartGrid();
  }

  reconnect(): void {
    if (this.source()) this.restartGrid();
  }

  disconnect(): void {
    this.feed?.stop();
  }

  /** Snapshot-only REST sources: re-fetch and diff the snapshot into the grid in place. */
  refreshSnapshot(): void {
    void this.feed?.refresh();
  }

  // ── source picker ──

  /** Switch this blotter instance to another source (drives the URL so it persists in layouts). */
  switchSource(src: BlotterSource): void {
    this.picker().hide();
    if (src.id === this.source()?.id) return;
    this.activeSource.open(src);
    this.router.navigate(['/blotter'], { queryParams: { source: src.id } });
  }

  newSource(): void {
    this.picker().hide();
    this.dialog().show();
  }

  editSource(src: BlotterSource, event: Event): void {
    event.stopPropagation();
    this.picker().hide();
    this.dialog().show(src);
  }

  /** Catalog sources are read-only — open the dialog pre-filled to save an editable ad-hoc copy. */
  duplicateSource(src: BlotterSource, event: Event): void {
    event.stopPropagation();
    this.picker().hide();
    this.dialog().show(src);
  }

  /** Delete an ad-hoc source; if it's the one on screen, drop back to the empty-state picker. */
  removeSource(src: BlotterSource, event: Event): void {
    event.stopPropagation();
    const wasActive = src.id === this.source()?.id;
    this.repo.removeAdhoc(src.id);
    if (wasActive) {
      this.activeSource.clear();
      this.router.navigate(['/blotter']);
    }
  }

  /** A new/edited ad-hoc source was saved from the dialog — open it. */
  onSourceSaved(src: BlotterSource): void {
    if (src.id === this.source()?.id) {
      // Edited the active source in place — switchSource would no-op on the same id, leaving the
      // old connection settings and roll-up running. Re-apply the saved source directly.
      this.applySource(src);
      return;
    }
    this.switchSource(src);
  }

  private applySource(src: BlotterSource | null): void {
    this.feedUnsub?.();
    this.feed?.stop();
    this.source.set(src);
    this.rollup.set(src?.rollup ?? null);
    this.rollupOn.set(!!src?.rollup?.enabled);
    if (!src) {
      this.showGrid.set(false);
      return;
    }
    this.columnMode.set(src.columnMode);
    this.restartGrid();
  }

  /** Remount the grid (setInitialRowData is one-shot per grid instance), then start the feed. */
  private restartGrid(): void {
    this.feedUnsub?.();
    this.feed?.stop();
    // Drop the old feed's columns now so the new grid doesn't mount with stale defs that the
    // feed start then clears (a visible flash, and a spurious group-removal event when rolled up).
    this.columns.set([]);
    this.showGrid.set(false);
    setTimeout(() => {
      this.showGrid.set(true);
      setTimeout(() => this.startFeed(), 0);
    }, 0);
  }

  private startFeed(): void {
    const src = this.source();
    const grid = this.grid();
    if (!src || !grid) return;
    this.columns.set([]);
    // Adapt the Angular grid's Subjects/method to the framework-free GridOps interface.
    const ops: GridOps = {
      addRows: (rows) => grid.addRows$.next(rows),
      updateRows: (rows) => grid.updateRows$.next(rows),
      deleteRows: (rows) => grid.deleteRows$.next(rows),
      setInitialRowData: (rows) => grid.setInitialRowData(rows),
    };
    const feed = new BlotterFeed(src, ops, (rec) => this.onFirstRecord(rec));
    this.feed = feed;
    this.feedState.set(feed.getState());
    this.feedUnsub = feed.subscribe(() => this.feedState.set(feed.getState()));
    feed.start();
  }

  private onFirstRecord(rec: Row): void {
    const src = this.source();
    const { columns, formats } = inferColumns(rec, src?.keyField);
    this.formats.set(formats);
    if (this.columnMode() === 'infer') this.columns.set(columns);
    // An explicit source hierarchy wins (completed with inferred aggregations for the fields it
    // doesn't cover); otherwise — including "open rolled up" saved with a blank hierarchy —
    // suggest one from the inferred columns so the toolbar toggle works on any payload with
    // plausible grouping fields.
    if (src?.rollup?.groupBy.length) {
      this.rollup.set(completeRollup(src.rollup, columns));
    } else {
      this.rollup.set(suggestRollup(columns, { keyField: src?.keyField, mode: src?.mode }));
    }
  }

  modeSeverity(mode: BlotterMode): 'success' | 'warn' | 'danger' {
    return mode === 'streaming' ? 'danger' : mode === 'append' ? 'warn' : 'success';
  }
}
