...
- Dock (Dock 3.0): `dock-config.json` — `favorites[]` entries are `item`s (launch buttons) or `folder`s, which render as dock DROPDOWNS whose children come from the `contentMenu[]` folder with the SAME id (Dock 3.0 folder merging; current bar: **Showcase** = all sample apps + **Prism Blotters**). Icons accept a URL or `{light, dark}` theme variants. Dock3Service also enables `providerIconContentMenu` and a "Macro Tools" more-menu (Analytics Dashboard / Process Manager). **LOB dock apps**: lines of business publish custom dock entries via the storage API's shared `/dock-apps` resource (`{id, label, iconUrl, type: "icon"|"dropdown", url | children[], lob, sortOrder}`) — the platform reads them at dock init and merges them fail-soft with `lob:`-namespaced ids (`icon` → dock button + "LOB Apps" content catalog grouped by `lob`; `dropdown` → dock folder). Storefront: `storefront-config.json` — nav = **Capital Markets** (FX / Rates / Commodities / Risk / Spread, populated by LOB apps published via the storage API's shared `/store-apps`; items match `category` OR any tag, case-insensitively) + **Desk & Library** (Middle Office / Blotters / Showcase / All Apps). ALL registry apps carry category `Showcase` (business-area categories are LOB-owned; Prism keeps `Blotters`). `cardClickBehavior: "show-app-details"` — clicking a card body opens the app-details page (description, screenshots from `public/screenshots/`, publisher + contact/support) while the Launch button launches; cards carry ☆ Favorite + 📌 Add to Dock secondary buttons (dock pins = per-user `dock-pinned-apps` preference → `pin:`-namespaced dock favorites, applied live via Dock 3.0 `updateConfig`). Content-menu ⭐ stars: the dock UI only FIRES `bookmark-content-menu-entry` on a star click — the provider override owns the whole toggle (add/remove the same-id favorite, re-derive `bookmarked` flags, push `updateConfig` for the live repaint — `save-config` alone never repaints) and persists the ids in the per-user `dock-bookmarks` preference (serialized writes; boot-unresolved ids unioned back so a feed outage never erases them; restored at init). The star means "on the dock": it also fills for Storefront-pinned apps, ONE app never gets two bar buttons (a `pin:` button is suppressed while a bookmark favorite covers the same appId), and un-starring removes the app from the dock entirely — bookmark AND pin (the unpin is delegated to StoreService, the pin owner; right-click bar removals of `pin:` favorites, which arrive via `save-config`, delegate the same way). Store cards render the 📌 button from the SAME unified state (`isOnDock` = pinned OR bookmarked, via `Dock3Service.getBookmarkedAppIds()`), "Remove from Dock" removes both affordances (`removeBookmarksForApp` + unpin), and the intent-carrying action ids (`dock-pin-add`/`dock-pin-remove`) make a stale card click no-op instead of inverting. A store window already open during a dock-side star/unpin shows the old button until the store re-renders (next open/navigation) — cards cannot be pushed without their `primaryButton`. The dock UI renders the star from the entry's `bookmarked` flag ONLY — never favorites membership — and never clears it itself. Snap: `snap-config.json`. Entitlements: `entitlements.json`. `settings.json` holds `platformSettings`, optional `browserSettings` (Workspace v24 browser options: `allowDuplicatePageTitles`, `indicators` suppression, `tabSearchButton`), and the `storage` block (unified-storage environments, below).
...
- Workspace Storage reference API: `http://localhost:3000/workspace/v1` — phase-1 implementation of the unified storage contract (workspaces / pages / dock / favorites / preferences per `X-User-Id`, `/config/*` passthrough, and the shared LOB registries `/dock-apps` (dock buttons/dropdowns) + `/store-apps` (full store cards incl. details-page fields) — validated PUTs, not user-scoped; ETag/If-Match, RFC 9457 `problem+json` errors, debounced file persistence to `.workspace-store.json`). Contract: OpenAPI 1.2.0 + Postman collection in `docs/api/`
...
5. Register the app in `public/{local,openshift}/apps.json` — add an entry with `appId`, `manifest` → the view-manifest URL, `manifestType: "view"`, `icons`, `tags`, and a `category`. This alone surfaces it in the store, home search, and as a launch target. Registry apps use category `Showcase` (Prism uses `Blotters`) — the business-area categories (FX / Rates / Commodities / Risk / Spread / Middle Office) are LOB-owned, populated via the storage API's `/store-apps`.
6. Add a dock entry in `public/{local,openshift}/dock-config.json` — sample apps go INSIDE the `showcase` content-menu folder (its sub-folders by framework); only give an app its own `favorites[]` entry/folder when it warrants top-level dock real estate (folder favorites need a same-id `contentMenu` folder for their children). Then storefront nav usually needs NO change (the Showcase item already covers registry apps; nav items match `category` OR tags case-insensitively, and OpenFin caps nav at 3 sections / 5 items with the dynamic Favorites section occupying one slot), and add an icon under `public/icons/`.
