...
export function registerAddDataConnectivityPrompt(server: McpServer): void {
  server.prompt(
    ...
    'Generate real-time data connectivity code (AMPS, Solace, NATS, or WebSocket) with conflation and grid wiring',
    {
      framework: z.enum(['angular', 'react']).describe('Target framework'),
      transport: z.enum(TRANSPORTS).describe('Message transport: amps, solace, nats, or websocket'),
...
function getTransportSetup(transport: string, topic: string, framework: string): TransportSetup {
  ...
  switch (transport) {
    case 'amps':
      return {
        imports: "import { AmpsTransport } from '@macro/transports';",
        connection: `${indent}const client = new AmpsTransport('${framework}-client');
${indent}client.connect({ url: 'ws://localhost:9100/amps/json' }).then(async () => {
...
${framework === 'angular' ? `${indent}// Store client reference for cleanup` : ''}`,
        cleanup: framework === 'angular'
          ...
          : '      sub?.unsubscribe();\n      // client.disconnect();',
        notes: `## AMPS Notes
- Default AMPS endpoint: \`ws://localhost:9100/amps/json\` (passed as \`connect({ url })\`)
- Parse JSON with \`msg.json<T>()\`; the raw payload string is \`msg.data\`
- Supports SOW (State of the World) queries: \`client.sow(callback, topic, filter)\`
- Angular DI: \`import { AmpsTransportService } from '@macro/transports/angular'\`
- React hook: \`import { useAmpsTransport } from '@macro/transports/react'\`
- See \`get_library_api\` tool with \`library: "transports"\` for full API reference`,
      };

    case 'solace':
      return {
        imports: "import { SolaceTransport, type SolaceConnectionOptions } from '@macro/transports';",
        connection: `${indent}const client = new SolaceTransport('${framework}-client');
...
${indent}});`,
        cleanup: framework === 'angular'
          ...
          : '      sub?.unsubscribe();\n      // client.disconnect();',
        notes: `## Solace Notes
- Connect with \`{ hostUrl, vpnName, userName, password }\` (\`SolaceConnectionOptions\`)
- Parse JSON with \`msg.json<T>()\`; the raw payload string is \`msg.data\`
...
- Angular DI: \`import { SolaceTransportService } from '@macro/transports/angular'\`
- React hook: \`import { useSolaceTransport } from '@macro/transports/react'\`
- See \`get_library_api\` tool with \`library: "transports"\` for full API reference`,
      };
...
${indent}})${subEnd};`,
        cleanup: framework === 'angular'
          ...
          : '      sub?.unsubscribe();\n      // client.disconnect();',
        notes: `## NATS Notes
- Uses the unified @macro/transports library
- Parse JSON with \`msg.json<T>()\`; the raw payload string is \`msg.data\`
