...
${colDefs}
  ...
  ngOnInit(): void {
    ...
    // Example with AMPS (@macro/transports):
    //   const amps = new AmpsTransport('my-app');
    //   await amps.connect({ url: 'ws://localhost:9100/amps/json' });
    //   const { observable } = await amps.subscribeAsObservable('your-topic');
    //   this.subscriptions.add(
    //     observable.subscribe(msg => this.grid.updateRows$.next([msg.json()]))
    //   );
    //
...
5. The grid auto-applies \`buildAgGridTheme(isDark)\` (a \`themeQuartz\` theme) from \`@macro/macro-design\`, watching the \`.dark\` class — no manual theming needed
...
export function ${componentName}() {
  ...
  useEffect(() => {
    ...
    //     gridRef.current?.updateRows$.next([msg.json()]);
    //   });
