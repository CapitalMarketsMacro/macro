...
import { findWorkspaceRoot, getWorkspacePort } from './openfin-registration';
...
export function registerListIcons(server: McpServer): void {
  server.tool(
    ...
    async ({ category, search }) => {
      ...
      const wsPort = getWorkspacePort(root);
      let output = `# Capital Markets Icons (${available.length} available)\n\n`;
      output += `Use these icon names with the \`import_figma_app\` / \`register_openfin_app\` tools' \`icon\` parameter.\n`;
      output += `Each icon has dark + light variants served at:\n`;
      output += `  - http://localhost:${wsPort}/icons/capital-markets/dark/{name}.svg\n`;
      output += `  - http://localhost:${wsPort}/icons/capital-markets/light/{name}.svg\n\n`;

      const lowerSearch = search?.toLowerCase();
      const lowerCategory = category?.toLowerCase();

      // Track which available icons we've shown so uncategorized ones aren't hidden.
      ...
      for (const [cat, icons] of Object.entries(CATEGORIES)) {
        ...
        filtered.forEach(i => shown.add(i));
        output += `## ${cat}\n`;
        output += filtered.map(i => `  - ${i}`).join('\n');
        output += '\n\n';
      }

      // "Other" — any available icon not placed in a category (so the list is exhaustive).
      const categorized = new Set(Object.values(CATEGORIES).flat());
      let other = available.filter(i => !categorized.has(i));
      if (lowerSearch) other = other.filter(i => i.includes(lowerSearch));
      // Only show Other when not filtering to a specific (non-"other") category.
      const wantsOther = !lowerCategory || 'other'.includes(lowerCategory) || 'uncategorized'.includes(lowerCategory);
      if (other.length > 0 && wantsOther) {
        output += `## Other\n`;
        output += other.map(i => `  - ${i}`).join('\n');
        output += '\n\n';
      }
