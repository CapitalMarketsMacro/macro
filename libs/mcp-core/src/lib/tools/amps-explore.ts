...
// Dynamic require hidden from esbuild's static analysis
const _require = typeof require !== 'undefined' ? require : (m: string) => { throw new Error(`Cannot require ${m}`); };
function getAmpsClient(): any {
  const mod = 'amps';
  const amps = _require(mod);
  return amps.Client;
}
  ...
  if (topic && client) {
    try {
      ...
      await new Promise<void>((resolve) => {
        ...
        client.sowAndSubscribe(
          (msg: any) => {
            const cmd = msg.header?.command?.() || '';
