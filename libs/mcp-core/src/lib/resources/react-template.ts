...
## 2. app.tsx
Theme state comes from the shared \`useTheme()\` hook (\`@macro/macro-design/react\`).
\`basename\` lets the app be embedded under \`/<app>/\` in OpenFin.
\`\`\`tsx
...
## 4. vite.config.ts
Note \`base\` (for OpenFin embedding under \`/<app>/\`) and the full \`@macro/*\` alias
block — longer subpath keys (\`/react\`, \`/theme-sync\`) must come BEFORE their parents.
\`\`\`typescript
...
export default defineConfig(() => ({
  ...
  test: {
    ...
    reporters: ['default'],
    coverage: {
      reportsDirectory: '../../coverage/apps/my-react-app',
