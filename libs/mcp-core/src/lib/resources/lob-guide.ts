...
// Messaging (pick your transport — all live in @macro/transports)
...
OpenFin manifests live in two environment folders that stay in sync:
...
In **both** folders, add your app to \`manifest.fin.json\` and \`settings.json\` under \`customSettings.apps\`:
...
\`\`\`json
{
  ...
  "manifest": "http://localhost:4202/local/my-lob-app.fin.json",
  "manifestType": "view",
...
Create the view manifest \`my-lob-app.fin.json\` (in both \`local/\` and \`openshift/\`) and add a Dock favorite under \`customSettings.dock3.favorites\`:
...
> **Tip:** Use the \`register_openfin_app\` MCP tool to write the view manifest, the \`manifest.fin.json\`/\`settings.json\` entries, and the Dock favorite in **both** \`local/\` and \`openshift/\` automatically. New LOB apps use ports 4204+ (the macro-angular-fdc3 demo uses 4203).
...
const amps = new AmpsTransport('my-lob-fx');
await amps.connect({ url: 'ws://amps-server:9100/amps/json' });

const conflation = new ConflationSubject<string, FxRate>(100);  // 100ms

const { observable } = await amps.subscribeAsObservable('/topic/fx-rates');
observable.subscribe((msg) => {
  const rate = msg.json<FxRate>();
