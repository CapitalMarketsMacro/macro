...
> \`AmpsClient\` / \`SolaceClient\` / \`NatsClient\` are aliases for the \`*Transport\` classes. The old standalone \`@macro/amps\`, \`@macro/solace\`, and \`@macro/nats\` libraries have been removed — everything lives in \`@macro/transports\`.

### Messages

Every transport delivers a unified \`TransportMessage\`:
\`\`\`typescript
interface TransportMessage {
  topic: string;
  data: string;        // raw payload string
  json<T>(): T;        // parsed JSON
  reply?: string;
  raw?: unknown;
}
\`\`\`
Parse JSON with \`msg.json<T>()\`; the raw string is \`msg.data\`.

### AMPS (unified)
\`\`\`typescript
import { AmpsTransport } from '@macro/transports';
const client = new AmpsTransport('my-app');
await client.connect({ url: 'ws://localhost:9100/amps/json' });
